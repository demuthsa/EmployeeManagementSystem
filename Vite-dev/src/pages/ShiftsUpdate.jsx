...
export default function ShiftsUpdate() {

    const location = useLocation();
    console.log(location.state.row);
    // make an easier variable for the passed state values
    const row = location.state.row;

    const checkShiftInit = (start) => {
        if (start === "20:00:00") {
            // initial state is night
            return "night";
        } else {
            return "day";
        }
    ...
    const [day_of_week_input, setDay] = useState(row.day_of_week);
    const [start_date_input, setStartDate] = useState(row.start_date);
    const [end_date_input, setEndDate] = useState(row.end_date);
    const [start_time_input, setStartTime] = useState(row.start_time);
    const [end_time_input, setEndTime] = useState(row.end_time);
    const [shift_input, setShift] = useState(checkShiftInit(row.start_time));
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.put(`http://flip3.engr.oregonstate.edu:1819/shifts/${row.shift_id}`, {
                day_of_week_input,
                start_date_input,
                end_date_input,
                start_time_input,
    ...
    return (
        <div>
            <h3>Update the Shift With ID: {row.shift_id}</h3>
